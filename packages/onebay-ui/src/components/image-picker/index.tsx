...
const ImagePicker = defineComponent({
  ...
  setup(props) {
    const rootCls = computed(() => {
      return classNames('ob-image-picker', props.className)
    })
      ...
      <div class={rootCls} style={style}>
        {matrix.map((row, i) => (
          <div class="ob-image-picker__flex-box" key={`${i + 1}`}>
            {row.map((item, j) =>
              item.url ? (
                <div class="ob-image-picker__flex-item" key={`img${i * length + j}`}>
                  <div class="ob-image-picker__item">
                    <div
                      class="ob-image-picker__remove-btn"
                      onClick={() => handleRemoveImg(i * length + j)}></div>
                    <img
                      class="ob-image-picker__preview-img"
                      src={item.url}
                ...
                <div class="ob-image-picker__flex-item" key={`btn${i * length + j}`}>
                  {item.type === 'btn' ? (
                    <div
                      class="ob-image-picker__item ob-image-picker__choose-btn"
                      onClick={chooseFile}>
