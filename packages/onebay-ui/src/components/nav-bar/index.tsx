  ...
  setup(props: NavBarProps) {
    ...
    const rootCls = computed(() => {
      ...
      return classNames(
        {
          'ob-nav-bar': true,
          'ob-nav-bar--fixed': fixed,
          'ob-nav-bar--no-border': !border
        },
        className
      )
    })
  ...
  render(): JSX.Element {
    ...
    return (
      <div class={rootCls} style={customStyle}>
        ...
        <div class="ob-nav-bar__title">{title || ($slots.default && $slots.default())}</div>
        <div class="ob-nav-bar__right-view">{rightContent || ($slots.right && $slots.right())}</div>
      </div>
    )
  }
})
