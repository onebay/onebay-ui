...
export default defineComponent({
  ...
  setup(props) {
    ...
    const rootCls = computed(() => {
      const rootClassName = ['ob-pagination']
      const classObject = {
        'ob-pagination--icon': props.icon
      }
  ...
  render(): JSX.Element {
    ...
    return (
      <div class={rootCls} style={style}>
        <div class="ob-pagination__btn-prev">
          {icon ? (
            <Button onClick={onPrev} size="small" disabled={prevDisabled}>
              <div class="ob-icon ob-icon-chevron-left text" />
            </Button>
          ) : (
            <Button onClick={onPrev} size="small" disabled={prevDisabled}>
              {preText}
        ...
        <div class="ob-pagination__number">
          <span class="ob-pagination__number-current text">{state.currentPage}</span>/
          {state.maxPage}
        </div>
        <div class="ob-pagination__btn-next">
          {icon ? (
            <Button onClick={onNext} size="small" disabled={nextDisabled}>
              <div class="ob-icon ob-icon-chevron-right text" />
            </Button>
          ) : (
            <Button onClick={onNext} size="small" disabled={nextDisabled}>
              {nextText}
