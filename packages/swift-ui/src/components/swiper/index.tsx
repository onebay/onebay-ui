...
export default defineComponent({
  ...
  methods: {
    ...
    renderLazyDom(slideEls) {
      slideEls.forEach((item) => {
        if (item.className != 'at-swiper-slide at-swiper-slide-selected') {
          const dom = document.createElement('div')
          //jd 图片占位图)
          if (this.lazyLoadUrl) {
            dom.style.backgroundImage = `url('${this.lazyLoadUrl}')`
            dom.className = 'at-lazy img'
          } else {
            dom.className = 'at-lazy preloader'
          }
    ...
    updateEvent() {
      this.$nextTick(() => {
        this.domTimer = setTimeout(() => {
          this.swiperWrap = this.$el.querySelector('.at-swiper-wrap')
    ...
    _slideClassHandle() {
      const selectedSlide = this.$el.querySelector('.at-swiper-slide-selected')
      selectedSlide && selectedSlide.classList.remove('at-swiper-slide-selected')
      this.slideEls[this.currentPage - 1].classList.add('at-swiper-slide-selected')
    ...
    _createLoop() {
      const swiperWrapEl = this.$el.querySelector('.at-swiper-wrap')
    ...
    _imgLazyLoad(type) {
      ...
      let imgLazyLoadEl
      requestAniFrame(() => {
        imgLazyLoadEl = this.swiperWrap.querySelectorAll('.at-img-lazyload')
        if (type == 1) {
          imgLazyLoadEl = this.slideEls[this.currentPage - 1].querySelectorAll('.at-img-lazyload')
        }
        ...
        imgLazyLoadEl.forEach((item) => {
          ...
          /* istanbul ignore next */
          img.onload = () => {
            const lazyLoadingEle = item.parentElement.querySelector('.at-lazy')
            if (lazyLoadingEle) {
              item.parentElement.removeChild(lazyLoadingEle)
            }
            item.src = src
            item.classList.remove('at-img-lazyload')
          }
          /* istanbul ignore next */
          img.onerror = () => {
  ...
  render() {
    ...
    return (
      <div
        class={classNames('at-swiper', direction, { dragging: dragging })}
        onTouchstart={_onTouchStart}
        onMousedown={_onTouchStart}>
        <div
          class="at-swiper-wrap"
          style={{
        ...
        {paginationVisible && (
          <div class="at-swiper-pagination">
