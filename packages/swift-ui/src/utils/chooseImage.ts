import { shouleBeObject, getParameterError } from './index'
...
const chooseImage = function (options: ChooseImageOptions): Promise<ChooseImageResponse> {
  ...
  const { count = 1, success, fail, complete, imageId = 'chooseImage' } = options
  ...
  let chooseImageId = document.getElementById(imageId) as HTMLInputElement
  if (!chooseImageId) {
    ...
    chooseImageId = document.getElementById(imageId) as HTMLInputElement
  }
  let chooseImageCallback: any
  const chooseImagePromise = new Promise<ChooseImageResponse>((resolve) => {
