...
describe('Swiper Event', () => {
  it('Swiper touch', async () => {
    ...
    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 10, pageY: 60 }] })
  })
  ...
  it('Swiper touch with component with lazyLoad', async () => {
    ...
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('transitionend')
    await sleep(20)
    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 10, pageY: 60 }] })
  ...
  it('Swiper touch with component loop = true', async () => {
    ...
    await sleep(20)
    wrapper.trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 14, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('transitionend')
    await sleep(20)
  ...
  it('Swiper touch prev page', async () => {
    ...
    await sleep(20)
    wrapper.trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 10, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 60, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 60, pageY: 60 }] })
    await sleep(20)
  })
  ...
  it('Swiper touch with component', async () => {
    ...
    await sleep(20)
    wrapper.trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 20, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 10, pageY: 40 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 10, pageY: 40 }] })
  })
  ...
  it('Swiper touch with lazyLoad image', async () => {
      ...
      <Swiper swiperData={imgs} direction="horizontal" lazyLoad={true}>
        {
          imgs.map((item, index) => (
            <div key={index} class="at-swiper-slide">
              <img data-src={item.imgSrc} style="max-width: 100%" class="at-img-lazyload" />
            </div>
          ))
        }
      </Swiper>
    ...
    await sleep(20)
    wrapper.trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 60 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 0, pageY: 0 }] })
    await sleep(20)
    expect(wrapper.element).toMatchSnapshot()

    wrapper.find('.at-swiper-wrap').trigger('touchstart', { changedTouches: [{ pageX: 100, pageY: 40 }] })
    wrapper.vm.dragging = false
    console.log(1111, wrapper.vm.dragging);
    wrapper.find('.at-swiper-wrap').trigger('touchmove', { changedTouches: [{ pageX: 0, pageY: 40 }] })
    wrapper.find('.at-swiper-wrap').trigger('touchend', { changedTouches: [{ pageX: 0, pageY: 40 }] })
  })
})
